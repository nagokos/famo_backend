        ...
        <v-row>
          <template v-if="$route.path.includes('/send_password_reset')">
            ...
            >
              ...
              <p :class="$vuetify.breakpoint.mobile ? 'text-caption mt-6' : 'text-subtitle-2 mt-6'">
          ...
          <template v-if="$route.path.includes('/password_reset')">
            ...
            >
              <ValidationProvider
                ...
                :rules="{ required: true, min: 8, formFormat: /^(?=.*?[a-z])(?=.*?\d)[a-z\d]{8,}$/i, passwordConfirmed: 'password' }"
                name="パスワード確認"
              >
                <v-text-field
          ...
          <v-col class="pt-0">
            <v-btn
              ...
              @click="$route.path.includes('/password_reset') ? handleSubmit(resetPassword) : handleSubmit(sendEmail)"
            >
  ...
  mounted() {
    if (this.$route.path.includes("/password_reset")) {
      if (!document.cookie.includes("reset_token")) {
        this.$router.push({ name: "passwordReset", params: { type: "send_password_reset" } })
      }
    }
  },
  methods: {
