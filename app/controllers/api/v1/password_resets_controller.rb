  ...
  def edit
    ...
    User.load_from_reset_password_token(params[:id]) do |user, failure_reason|
      if user && !failure_reason
        cookies[:reset_token] = { value: token, path: Settings.url.password_reset,
                                  expires: 5.minutes.from_now, secure: Rails.env.production?, same_site: 'Lax' }
        redirect_to Settings.url.password_reset
      else
        case failure_reason
  ...
  def update
    ...
    if (user = User.load_from_reset_password_token(params[:id]))
      ...
      if user.change_password(params[:user][:password])
        # クッキーを削除
        cookies[:reset_token] = { value: token, path: Settings.url.password_reset, expires: 50.years.ago }
        head :ok
      else
